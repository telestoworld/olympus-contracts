...
contract TelestoCVXBondDepository is Ownable {
    ...
    address public immutable treasury; // mints TELO when receives principal
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( TELO ).safeTransfer( _recipient, _amount ); // send payout
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principal or TELO) to the DAO
     *  @return bool
     */
