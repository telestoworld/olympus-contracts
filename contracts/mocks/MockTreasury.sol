...
contract MockTelestoTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sTELOQueue; // Delays change to sTELO address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sTELO).balanceOf(msg.sender); // Can only borrow against sTELO held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithTELO(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(TELO).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.STELO) {
      // 9
